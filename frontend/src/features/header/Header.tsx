...
const Header = (props: Props) => {
    ...
    const signInBtn = !props.auth.isAuthenticated && <SignInButton />
    const userHeading = props.auth.isAuthenticated && (
        <Navbar.Group>
            <Icon icon='user' iconSize={20} style={{marginRight: '7px'}}/>
            <NavbarHeading className='bp3-minimal' >{props.auth.email}</NavbarHeading>
        </Navbar.Group>
    )
    return (
